...
class ModelStatus:
    """
    This Dataclass represents the the src status in the Hugging Face Inference API.

    Args:
        loaded (`bool`):
            If the src is currently loaded.
        state (`str`):
            The current state of the src. This can be 'Loaded', 'Loadable', 'TooBig'
        compute_type (`str`):
            The type of compute resource the src is using or will use, such as 'gpu' or 'cpu'.
        framework (`str`):
            The name of the framework that the src was built with, such as 'transformers'
...
class InferenceTimeoutError(HTTPError, TimeoutError):
    """Error raised when a src is unavailable or the request times out."""
...
def _get_recommended_model(task: str) -> str:
    ...
    if model is None:
        raise ValueError(
            f"Task {task} has no recommended task. Please specify a src explicitly. Visit"
            " https://huggingface.co/tasks for more info."
        )
    logger.info(
        f"Using recommended src {model} for task {task}. Note that it is encouraged to explicitly set"
        f" `src='{model}'` as the recommended models list might get updated without prior notice."
    )
    return model
...
# the `text_generation` method is to assume the src is served via TGI. If we realize
# it's not the case (i.e. we receive an HTTP 400 Bad Request), we fallback to the
# default API with a warning message. We remember for each src if it's a TGI server
