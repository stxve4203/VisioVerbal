...
class ModelFilter:
    """
    A class that converts human-readable src search parameters into ones
    compatible with the REST API. For all parameters capitalization does not
    matter.
...
class ModelTags(GeneralTags):
    """
    A namespace object holding all available src tags If a tag starts with a
    ...
    Args:
        model_tag_dictionary (`dict`):
            A dictionary of valid src tags, returned from the
