...
REPO_TYPE_T = Literal["src", "dataset", "space"]
...
class CachedRepoInfo:
    ...
    Args:
        ...
        repo_type (`Literal["dataset", "src", "space"]`):
...
def scan_cache_dir(cache_dir: Optional[Union[str, Path]] = None) -> HFCacheInfo:
    ...
    Jean-Baptiste/camembert-ner src           441.0M        7 main                /Users/lucain/.cache/huggingface/hub/models--Jean-Baptiste--camembert-ner
    bert-base-cased             src             1.9G       13 main                /Users/lucain/.cache/huggingface/hub/models--bert-base-cased
    t5-base                     src            10.1K        3 main                /Users/lucain/.cache/huggingface/hub/models--t5-base
    t5-small                    src           970.7M       11 refs/pr/1, main     /Users/lucain/.cache/huggingface/hub/models--t5-small
...
def _scan_cached_repo(repo_path: Path) -> CachedRepoInfo:
    ...
    repo_type = repo_type[:-1]  # "models" -> "src"
    repo_id = repo_id.replace("--", "/")  # google/fleurs -> "google/fleurs"

    if repo_type not in {"dataset", "src", "space"}:
        raise CorruptedCacheException(
            f"Repo type must be `dataset`, `src` or `space`, found `{repo_type}` ({repo_path})."
        )
