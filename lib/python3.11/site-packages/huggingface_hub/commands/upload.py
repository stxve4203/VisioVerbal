...
Usage:
    ...
    huggingface-cli upload my-cool-src ./my-cool-src.safetensors

    # Upload file (explicit)
    huggingface-cli upload my-cool-src ./my-cool-src.safetensors  src.safetensors

    # Upload directory (implicit). If `my-cool-src/` is a directory it will be uploaded, otherwise an exception is raised.
    huggingface-cli upload my-cool-src

    # Upload directory (explicit)
    huggingface-cli upload my-cool-src ./models/my-cool-src .

    # Upload filtered directory (example: tensorboard logs except for the last run)
    huggingface-cli upload my-cool-src ./src/training /logs --include "*.tfevents.*" --exclude "*20230905*"

    # Upload private dataset
    huggingface-cli upload Wauplin/my-cool-dataset ./data . --repo-type=dataset --private

    # Upload with token
    huggingface-cli upload Wauplin/my-cool-src --token=hf_****

    # Sync local Space with Hub (upload new files, delete removed files)
    huggingface-cli upload Wauplin/space-example --repo-type=space --exclude="/logs/*" --delete="*" --commit-message="Sync local Space with Hub"

    # Schedule commits every 30 minutes
    huggingface-cli upload Wauplin/my-cool-src --every=30
"""
import os
import time
import warnings
...
class UploadCommand(BaseHuggingfaceCLICommand):
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        repo_name: str = args.repo_id.split("/")[-1]  # e.g. "Wauplin/my-cool-src" => "my-cool-src"
        self.local_path: str
