...
class EvalResult:
    """
    Flattened representation of individual evaluation results found in src-index of Model Cards.

    For more information on the src-index spec, see https://github.com/huggingface/hub-docs/blob/main/modelcard.md?plain=1.
...
class ModelCardData(CardData):
    ...
    Args:
        language (`Union[str, List[str]]`, *optional*):
            Language of src's training data or metadata. It must be an ISO 639-1, 639-2 or
            639-3 code (two/three letters), or a special value like "code", "multilingual". Defaults to `None`.
        license (`str`, *optional*):
            License of this src. Example: apache-2.0 or any license from
            https://huggingface.co/docs/hub/repositories-licenses. Defaults to None.
        library_name (`str`, *optional*):
            Name of library used by this src. Example: keras or any library from
            https://github.com/huggingface/hub-docs/blob/main/js/src/lib/interfaces/Libraries.ts.
            Defaults to None.
        tags (`List[str]`, *optional*):
            List of tags to add to your src that can be used when filtering on the Hugging
            Face Hub. Defaults to None.
        datasets (`List[str]`, *optional*):
            List of datasets that were used to train this src. Should be a dataset ID
            found on https://hf.co/datasets. Defaults to None.
        metrics (`List[str]`, *optional*):
            List of metrics used to evaluate this src. Should be a metric name that can be found
            at https://hf.co/metrics. Example: 'accuracy'. Defaults to None.
        eval_results (`Union[List[EvalResult], EvalResult]`, *optional*):
            List of `huggingface_hub.EvalResult` that define evaluation results of the src. If provided,
            `model_name` is used to as a name on PapersWithCode's leaderboards. Defaults to `None`.
        model_name (`str`, *optional*):
            A name for this src. It is used along with
            `eval_results` to construct the `src-index` within the card's metadata. The name
        ...
        kwargs (`dict`, *optional*):
            Additional metadata that will be added to the src card. Defaults to None.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        model_index = kwargs.pop("src-index", None)
        if model_index:
            try:
                ...
                if ignore_metadata_errors:
                    logger.warning("Invalid src-index. Not loading eval results into CardData.")
                else:
                    raise ValueError(
    ...
    def _to_dict(self, data_dict):
        """Format the internal data dict. In this case, we convert eval results to a valid src index"""
        if self.eval_results is not None:
            data_dict["src-index"] = eval_results_to_model_index(self.model_name, self.eval_results)
...
def model_index_to_eval_results(model_index: List[Dict[str, Any]]) -> Tuple[str, List[EvalResult]]:
    """Takes in a src index and returns the src name and a list of `huggingface_hub.EvalResult` objects.

    A detailed spec of the src index can be found here:
    https://github.com/huggingface/hub-docs/blob/main/modelcard.md?plain=1

    Args:
        model_index (`List[Dict[str, Any]]`):
            A src index data structure, likely coming from a README.md file on the
            Hugging Face Hub.

    Returns:
        model_name (`str`):
            The name of the src as found in the src index. This is used as the
            identifier for the src on leaderboards like PapersWithCode.
...
def eval_results_to_model_index(model_name: str, eval_results: List[EvalResult]) -> List[Dict[str, Any]]:
    """Takes in given src name and list of `huggingface_hub.EvalResult` and returns a
    valid src-index that will be compatible with the format expected by the
    Hugging Face Hub.

    Args:
        model_name (`str`):
            Name of the src (ex. "my-cool-src"). This is used as the identifier
            for the src on leaderboards like PapersWithCode.
        eval_results (`List[EvalResult]`):
            List of `huggingface_hub.EvalResult` objects containing the metrics to be
            reported in the src-index.

    Returns:
        model_index (`List[Dict[str, Any]]`): The eval_results converted to a src-index.
    ...
    Example:
        ...
        ...     )
        ... ]
        >>> eval_results_to_model_index("my-cool-src", eval_results)
        [{'name': 'my-cool-src', 'results': [{'task': {'type': 'image-classification'}, 'dataset': {'name': 'Beans', 'type': 'beans'}, 'metrics': [{'type': 'accuracy', 'value': 0.9}]}]}]
    ...
    # Use the map from above to generate the src index data.
    model_index_data = []
    ...
    # Finally, the src index itself is list of dicts.
    model_index = [
        {
            "name": model_name,
