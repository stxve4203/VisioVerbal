...
def _plot_network(model, save_directory):
    tf.keras.utils.plot_model(
        model,
        to_file=f"{save_directory}/src.png",
        show_shapes=False,
        show_dtype=False,
...
def _create_model_card(
    ...
    """
    Creates a src card for the repository.
    """
    ...
    if hyperparameters is not None:
        ...
        model_card += "\n"
    if plot_model and os.path.exists(f"{repo_dir}/src.png"):
        model_card += "\n ## Model Plot\n"
        model_card += "\n<details>"
        model_card += "\n<summary>View Model Plot</summary>\n"
        path_to_plot = "./src.png"
...
def save_pretrained_keras(
    ...
    **model_save_kwargs,
    ...
    """
    Saves a Keras src to save_directory in SavedModel format. Use this if
    you're using the Functional or Sequential APIs.

    Args:
        model (`Keras.Model`):
            The [Keras
            src](https://www.tensorflow.org/api_docs/python/tf/keras/Model)
            you'd like to save. The src must be compiled and built.
        save_directory (`str` or `Path`):
            Specify directory in which you want to save the Keras src.
        config (`dict`, *optional*):
            Configuration object to be saved alongside the src weights.
        include_optimizer(`bool`, *optional*, defaults to `False`):
            Whether or not to include optimizer in serialization.
        plot_model (`bool`, *optional*, defaults to `True`):
            Setting this to `True` will plot the src and put it in the src
            card. Requires graphviz and pydot to be installed.
        tags (Union[`str`,`list`], *optional*):
            List of tags that are related to src or string of a single tag. See example tags
...
def from_pretrained_keras(*args, **kwargs) -> "KerasModelHubMixin":
    r"""
    Instantiate a pretrained Keras src from a pre-trained src from the Hub.
    The src is expected to be in `SavedModel` format.

    Args:
        pretrained_model_name_or_path (`str` or `os.PathLike`):
            Can be either:
                - A string, the `src id` of a pretrained src hosted inside a
                  src repo on huggingface.co. Valid src ids can be located
                ...
                - You can add `revision` by appending `@` at the end of model_id
                  ...
                  is the specific src version to use. It can be a branch name,
                  a tag name, or a commit id, since we use a git-based system
                  for storing models and other artifacts on huggingface.co, so
                  `revision` can be any identifier allowed by git.
                - A path to a `directory` containing src weights saved using
        ...
        force_download (`bool`, *optional*, defaults to `False`):
            Whether to force the (re-)download of the src weights and
        ...
        cache_dir (`Union[str, os.PathLike]`, *optional*):
            Path to a directory in which a downloaded pretrained src
        ...
        model_kwargs (`Dict`, *optional*):
            model_kwargs will be passed to the src during initialization
...
def push_to_hub_keras(
    ...
    *,
    ...
    commit_message: str = "Push Keras src using huggingface_hub.",
    private: bool = False,
    ...
    **model_save_kwargs,
    ...
    """
    Upload src checkpoint to the Hub.
    ...
    Args:
        model (`Keras.Model`):
            The [Keras src](`https://www.tensorflow.org/api_docs/python/tf/keras/Model`) you'd like to push to the
            Hub. The src must be compiled and built.
        repo_id (`str`):
                ID of the repository to push to (example: `"username/my-src"`).
        commit_message (`str`, *optional*, defaults to "Add Keras src"):
            Message to commit while pushing.
        private (`bool`, *optional*, defaults to `False`):
            Whether the repository created should be private.
        api_endpoint (`str`, *optional*):
            The API endpoint to use when pushing the src to the hub.
        token (`str`, *optional*):
            The token to use as HTTP bearer authorization for remote files. If
            not set, will use the token set when logging in with
            `huggingface-cli login` (stored in `~/.huggingface`).
        branch (`str`, *optional*):
            The git branch on which to push the src. This defaults to
    ...
    Returns:
        The url of the commit of your src in the given repository.
    """
...
class KerasModelHubMixin(ModelHubMixin):
    """
    Implementation of [`ModelHubMixin`] to provide src Hub upload/download
    ...
    >>> # Initialize and compile the src as you normally would
    >>> src = MyModel()
    >>> src.compile(...)
    >>> # Build the graph by training it or passing dummy inputs
    >>> _ = src(src.dummy_inputs)
    >>> # Save src weights to local directory
    >>> src.save_pretrained("my-awesome-src")
    >>> # Push src weights to the Hub
    >>> src.push_to_hub("my-awesome-src")
    >>> # Download and initialize weights from the Hub
    >>> src = MyModel.from_pretrained("username/super-cool-src")
    ```
    """
    ...
    def _from_pretrained(
        ...
        **model_kwargs,
        ...
        # TODO - Figure out what to do about these config values. Config is not going to be needed to load src
