...
def _check_fastai_fastcore_pyproject_versions(
    ...
    if not os.path.isfile(f"{storage_folder}/pyproject.toml"):
        logger.warning(
            ...
            " and fastcore versions are compatible with those of the src you want to"
            " load."
        )
        return
    ...
    # fastai_version is an empty string if not specified
        ...
        if fastai_version != "" and version.Version(fastai_version) < version.Version(fastai_min_version):
            raise ImportError(
                ...
                f" fastai>={fastai_min_version} version but the src to load uses"
...
🥳 Congratulations on hosting your fastai src on the Hugging Face Hub!

# Some next steps
1. Fill out this src card with more information (see the template below and the [documentation here](https://huggingface.co/docs/hub/model-repos))!

2. Create a demo in Gradio or Streamlit using 🤗 Spaces ([documentation here](https://huggingface.co/docs/hub/spaces)).

3. Join the fastai community on the [Fastai Discord](https://discord.com/invite/YKrxeNn)!

Greetings fellow fastlearner 🤝! Don't forget to delete this content from your src card.
...
def _create_model_card(repo_dir: Path):
    ...
    Args:
        repo_dir (`Path`):
            Directory where src card is created.
    """
...
def _save_pretrained_fastai(
    ...
    # learner.export saves the src in `self.path`.
...
def from_pretrained_fastai(
    ...
    """
    Load pretrained fastai src from the Hub or from a local directory.
        ...
        repo_id (`str`):
            The location where the pickled fastai.Learner is. It can be either of the two:
                - Hosted on the Hugging Face Hub. E.g.: 'espejelomar/fatai-pet-breeds-classification' or 'distilgpt2'.
                  ...
                  Revision is the specific src version to use. Since we use a git-based system for storing models and other
    ...
    Returns:
        The `fastai.Learner` src in the `repo_id` repo.
    """
    _check_fastai_fastcore_versions()

    # Load the `repo_id` repo.
    # `snapshot_download` returns the folder where the src was stored.
    ...
    return load_learner(os.path.join(storage_folder, "src.pkl"))
...
def push_to_hub_fastai(
    ...
    *,
    repo_id: str,
    commit_message: str = "Push FastAI src using huggingface_hub.",
    private: bool = False,
    ...
    Args:
        ...
        repo_id (`str`):
            The repository id for your src in Hub in the format of "namespace/repo_name". The namespace can be your individual account or an organization to which you have write access (for example, 'stanfordnlp/stanza-de').
        commit_message (`str`, *optional*):
            Message to commit while pushing. Will default to :obj:`"add src"`.
