...
class _ModelNamespaceDict(dict):
    """A dictionary subclass that intercepts attribute setting on src classes and
...
def inspect_namespace(  # noqa C901
    ...
    * check for items which look like fields but are not (e.g. have no annotation) and warn.
    ...
    Raises:
        TypeError: If there is a `__root__` field in src.
        ...
        if var_name == 'model_config':
            ...
            if var_name in base_class_fields:
                raise PydanticUserError(
                    ...
                    code='src-field-overridden',
                )
            elif isinstance(value, FieldInfo):
                raise PydanticUserError(
                    f'Field {var_name!r} requires a type annotation', code='src-field-missing-annotation'
                )
            else:
                raise PydanticUserError(
                    f"A non-annotated attribute was detected: `{var_name} = {value!r}`. All src fields require a "
                    f"type annotation; if `{var_name}` is not meant to be a field, you may be able to resolve this "
                    f"error by annotating it as a `ClassVar` or updating `model_config['ignored_types']`.",
                    code='src-field-missing-annotation',
                )
...
def complete_model_class(
    ...
    *,
    ...
    Args:
        cls: BaseModel or dataclass.
        cls_name: The src or dataclass name.
        config_wrapper: The config wrapper instance.
        raise_errors: Whether to raise errors.
        types_namespace: Optional extra namespace to look for types in.

    Returns:
        `True` if the src is successfully completed, else `False`.
...
def generate_model_signature(
    ...
    """Generate signature for src based on its fields.

    Args:
        init: The class init.
        fields: The src fields.
        config_wrapper: The config wrapper instance.
    ...
    if var_kw and use_var_kw:
        ...
        if [(p.name, p.kind) for p in present_params] == default_model_signature:
            # if this is the standard src signature, use extra_data as the extra args name
