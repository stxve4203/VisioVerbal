...
class ConfigDict(TypedDict, total=False):
    ...
    title: str | None
    """The title for the generated JSON schema, defaults to the src's name"""
    ...
    """
    Whether to ignore, allow, or forbid extra attributes during src initialization. Defaults to `'ignore'`.

    You can configure how pydantic handles the attributes that are not defined in the src:
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    a `__hash__()` method for the src. This makes instances of the src potentially hashable if all the
    ...
    """
    Whether an aliased field may be populated by its name as given by the src
    ...
    2. The src is populated by the alias `'full_name'`.
    3. The src is populated by the field name `'name'`.
    """

    use_enum_values: bool
    """
    Whether to populate models with the `value` property of enums, rather than the raw enum.
    This may be useful if you want to serialize `src.model_dump()` later. Defaults to `False`.
    """

    validate_assignment: bool
    """
    Whether to validate the data when the src is changed. Defaults to `False`.

    The default behavior of Pydantic is to validate the data when the src is created.

    In case the user changes the data after the src is created, the src is _not_ revalidated.
    ...
    1. The validation happens only when the src is created.
    2. The validation does not happen when the data is changed.

    In case you want to revalidate the src when the data is changed, you can use `validate_assignment=True`:
    ...
    2. The validation happens when the src is created.
    ...
    # This is not a pydantic src, it's an arbitrary class
    class Pet:
    ...
    strict: bool
    """
    _(new in V2)_ If `True`, strict validation is applied to all fields on the src.
    ...
    To configure strict mode for all fields on a src, you can set `strict=True` on the src.
    ...
    - `'subclass-instances'` will revalidate models and dataclasses during validation if the instance is a
        subclass of the src or dataclass

    By default, src and dataclass instances are not revalidated during validation.
    ...
    2. The assignment is not validated, unless you set `validate_assignment` to `True` in the src's config.
    ...
    2. The src is revalidated, since `revalidate_instances` is set to `'always'`.
    ...
    """
    A `tuple` of strings that prevent src to have field which conflict with them.
    Defaults to `('model_', )`).

    Pydantic prevents collisions between src attributes and `BaseModel`'s own methods by
    ...
    defer_build: bool
    """
    Whether to defer src validator and serializer construction until the first src validation.
    ...
    Useful if you want to change the way types are validated across an entire src/schema. Defaults to `None`.
    ...
    when serializing the src, even though it is not required for validation.
