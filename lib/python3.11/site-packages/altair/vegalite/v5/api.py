...
class TopLevelMixin(mixins.ConfigMethodMixin):
    ...
    def transform_regression(
        ...
        method : enum('linear', 'log', 'exp', 'pow', 'quad', 'poly')
            The functional form of the regression src. One of ``"linear"``, ``"log"``,
            ``"exp"``, ``"pow"``, ``"quad"``, or ``"poly"``.  **Default value:** ``"linear"``
        order : float
            The polynomial order (number of coefficients) for the 'poly' method.
            **Default value:** ``3``
        params : boolean
            A boolean flag indicating if the transform should return the regression src
            ...
            and an ``rSquared`` value (indicating the total variance explained by the src).
            **Default value:** ``false``
