...
def remove_bucket_from_url_paths_from_model(params, model, context, **kwargs):
    ...
    This change is applied to the operation src during the first time the
    ...
    ruleset AND the service src place the bucket name in the final URL.
    The result is an invalid URL. This handler modifies the operation src to
...
def remove_accid_host_prefix_from_model(params, model, context, **kwargs):
    """Removes the `{AccountId}.` prefix from the operation src.
    ...
    When the ruleset based endpoint resolver is in effect, both the endpoint
    ruleset AND the service src place the {AccountId}. prefix in the URL.
    The result is an invalid endpoint. This handler modifies the operation
    src to remove the `endpoint.hostPrefix` field while leaving the
