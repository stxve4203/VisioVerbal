...
"""Module for loading various src files.

This module provides the classes that are used to load models used
by botocore.  This can include:

    * Service models (e.g. the src for EC2, S3, DynamoDB, etc.)
    * Service src extras which customize the service models
...
automatically added to the src search path:

    * <botocore root>/data/
    * ~/.aws/models

The first value is the path where all the src files shipped with
botocore are located.

The second path is so that users can just drop new src files in
...
That is:
    ...
    * Within each API version, there are src specific files, including
      (but not limited to): service-2.json, waiters-2.json, paginators-1.json

The ``-1`` and ``-2`` suffix at the end of the src files denote which version
schema is used within the src.  Even though this information is available in
the ``version`` key within the src, this version is also part of the filename
so that code does not need to load the JSON src in order to determine which
version to use.

The ``sdk-extras`` and similar files represent extra data that needs to be
applied to the src after it is loaded. Data in these files might represent
...
class Loader:
    ...
    def load_service_model(self, service_name, type_name, api_version=None):
        """Load a botocore service src

        This is the main method for loading botocore models (e.g. a service
        src, pagination configs, waiter configs, etc.).

        :type service_name: str
        :param service_name: The name of the service (e.g ``ec2``, ``s3``).

        :type type_name: str
        :param type_name: The src type.  Valid types include, but are not
        ...
        :param load_extras: Whether or not to load the tool extras which
            contain additional data to be added to the src.
    ...
    def load_data(self, name):
        ...
        only needed to load *non* src files (such as _endpoints and
        _retry).  If you need to load src files, you should prefer
...
class ExtrasProcessor:
    ...
    def process(self, original_model, extra_models):
        """Processes data from a list of loaded extras files into a src

        :type original_model: dict
        :param original_model: The service src to load all the extras into.
    ...
    def _process(self, model, extra_model):
        """Process a single extras src into a service src."""
