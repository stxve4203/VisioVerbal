...
def getProfileName(profile):
    ...
    try:
        ...
        #    // name was "%s - %s" (src, manufacturer) || Description ,
        #    // but if the Model and Manufacturer were the same or the src
        #    // was long, Just the src,  in 1.x
...
def getProfileModel(profile):
    """
    (pyCMS) Gets the src for the given profile.

    If ``profile`` isn't a valid CmsProfile object or filename to a profile, a
    :exc:`PyCMSError` is raised.

    If an error occurs while trying to obtain the src tag,
    a :exc:`PyCMSError` is raised.
