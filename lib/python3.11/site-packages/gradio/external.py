...
def load(
    ...
    **kwargs,
    ...
    src repos (if src is "models") or Space repos (if src is "spaces"). The input
    and output components are automatically loaded from the repo.
    Parameters:
        name: the name of the src (e.g. "gpt2" or "facebook/bart-base") or space (e.g. "flax-community/spanish-gpt2"), can include the `src` as prefix (e.g. "models/facebook/bart-base")
        src: the source of the src: `models` or `spaces` (or leave empty if source is provided as a prefix in `name`)
        ...
        alias: optional string used as the name of the loaded src instead of the default name (only applies if loading a Space running Gradio 2.x)
    Returns:
        a Gradio Blocks object for the given src
    Example:
        import gradio as gr
...
def load_blocks_from_repo(
    ...
    **kwargs,
    ...
    """Creates and returns a Blocks instance from a Hugging Face src or Space repo."""
    if src is None:
        # Separate the repo type (e.g. "src") from repo name (e.g. "google/vit-base-patch16-224")
        tokens = name.split("/")
        assert (
            len(tokens) > 1
    ...
    factory_methods: dict[str, Callable] = {
        # for each repo type, we have a method that returns the Interface given the src name & optionally an api_key
    ...
    if hf_token is not None:
        if Context.hf_token is not None and Context.hf_token != hf_token:
            warnings.warn(
                """You are loading a src/Space with a different access token than the one you used to load a previous src/Space. This is not recommended, as it may cause unexpected behavior."""
            )
...
def from_model(model_name: str, hf_token: str | None, alias: str | None, **kwargs):
    ...
    print(f"Fetching src from: {model_url}")

    headers = {"Authorization": f"Bearer {hf_token}"} if hf_token is not None else {}

    # Checking if src exists, and if so, it gets the pipeline
    response = requests.request("GET", api_url, headers=headers)
    assert (
        response.status_code == 200
    ), f"Could not find src: {model_name}. If it is a private or gated src, please provide your Hugging Face access token (https://huggingface.co/settings/tokens) as the argument for the `api_key` parameter."
    p = response.json().get("pipeline_tag")
    pipelines = {
        "audio-classification": {
            # example src: ehcalabres/wav2vec2-lg-xlsr-en-speech-emotion-recognition
        ...
        "audio-to-audio": {
            # example src: facebook/xm_transformer_sm_all-en
            "inputs": components.Audio(
                source="upload", type="filepath", label="Input", render=False
            ),
        ...
        "automatic-speech-recognition": {
            # example src: facebook/wav2vec2-base-960h
            "inputs": components.Audio(
                source="upload", type="filepath", label="Input", render=False
            ),
        ...
        "feature-extraction": {
            # example src: julien-c/distilbert-feature-extraction
        ...
        "text-to-image": {
            # example src: osanseviero/BigGAN-deep-128
            "inputs": components.Textbox(label="Input", render=False),
        ...
        "token-classification": {
            # example src: huggingface-course/bert-finetuned-ner
            "inputs": components.Textbox(label="Input", render=False),
        ...
        "document-question-answering": {
            # example src: impira/layoutlm-document-qa
            "inputs": [
        ...
        "visual-question-answering": {
            # example src: dandelin/vilt-b32-finetuned-vqa
            "inputs": [
        ...
        "image-to-text": {
            # example src: Salesforce/blip-image-captioning-base
            "inputs": components.Image(
                type="filepath", label="Input Image", render=False
            ),
            "outputs": components.Textbox(label="Generated Text", render=False),
