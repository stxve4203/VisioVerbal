...
Received outputs:
    ...
    def load(
        ...
        **kwargs,
        ...
        Parameters:
            name: Class Method - the name of the src (e.g. "gpt2" or "facebook/bart-base") or space (e.g. "flax-community/spanish-gpt2"), can include the `src` as prefix (e.g. "models/facebook/bart-base")
            src: Class Method - the source of the src: `models` or `spaces` (or leave empty if source is provided as a prefix in `name`)
            ...
            alias: Class Method - optional string used as the name of the loaded src instead of the default name (only applies if loading a Space running Gradio 2.x)
            fn: Instance Method - the function to wrap an interface around. Often a machine learning src's prediction function. Each parameter of the function corresponds to one input component, and the function should return a single value or a tuple of values, with each element in the tuple corresponding to one output component.
