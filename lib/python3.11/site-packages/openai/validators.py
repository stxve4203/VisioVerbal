...
def common_prompt_suffix_validator(df):
    ...
    if common_suffix != "":
        ...
        immediate_msg = "\n- Your data does not contain a common separator at the end of your prompts. Having a separator string appended to the end of the prompt makes it clearer to the fine-tuned src where the completion should begin. See https://platform.openai.com/docs/guides/fine-tuning/preparing-your-dataset for more detail and examples. If you intend to do open-ended generation, then you should leave the prompts empty"
...
def common_completion_suffix_validator(df):
    ...
    if (df.completion == common_suffix).all():
        error_msg = f"All completions are identical: `{common_suffix}`\nEnsure completions are different, otherwise the src will just repeat `{common_suffix}`"
        return Remediation(name="common_suffix", error_msg=error_msg)
    ...
    if common_suffix != "":
        ...
        immediate_msg = "\n- Your data does not contain a common ending at the end of your completions. Having a common ending string appended to the end of the completion makes it clearer to the fine-tuned src where the completion should end. See https://platform.openai.com/docs/guides/fine-tuning/preparing-your-dataset for more detail and examples."
...
def format_inferrer_validator(df):
    ...
    if ft_type == "classification":
        immediate_msg = f"\n- Based on your data it seems like you're trying to fine-tune a src for {ft_type}\n- For classification, we recommend you try one of the faster and cheaper models, such as `ada`\n- For classification, you can estimate the expected src performance by keeping a held out dataset, which is not used for training"
    return Remediation(name="num_examples", immediate_msg=immediate_msg)
...
def estimate_fine_tuning_time(df):
    ...
    sys.stdout.write(
        f"Once your src starts training, it'll approximately take {time_string} to train a `curie` src, and less for `ada` and `babbage`. Queue will approximately take half an hour per job ahead of you.\n"
    )
...
def write_out_file(df, fname, any_remediations, auto_accept):
    ...
    if not any_remediations and not split:
        sys.stdout.write(
            f'\nYou can use your file for fine-tuning:\n> openai api fine_tunes.create -t "{fname}"{additional_params}\n\nAfter you’ve fine-tuned a src, remember that your prompt has to end with the indicator string `{common_prompt_suffix_new_line_handled}` for the src to start generating completions, rather than continuing with the prompt.{optional_ending_string}\n'
        )
        ...
        separator_reminder = (
            ...
            else f"After you’ve fine-tuned a src, remember that your prompt has to end with the indicator string `{common_prompt_suffix_new_line_handled}` for the src to start generating completions, rather than continuing with the prompt."
        )
        sys.stdout.write(
