...
class Completion:
    ...
    def create(cls, args):
        ...
        if args.engine and args.model:
            warnings.warn(
                "In most cases, you should not be specifying both engine and src."
            )
...
def api_register(parser):
    ...
    sub.add_argument(
        "-t",
        "--temperature",
        help="""What sampling temperature to use. Higher values means the src will take more risks. Try 0.9 for more creative applications, and 0 (argmax sampling) for ones with a well-defined answer.
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        ...
        "--src",
        required=False,
        help="A src (most commonly a src ID) to generate from. Defaults to the engine's default src.",
    )
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument(
        "-m",
        "--src",
        help="The src to use. At most one of `engine` or `src` should be specified.",
    )
    sub.add_argument(
        "--stream", help="Stream tokens as they're ready.", action="store_true"
    )
...
Mutually exclusive with `top_p`.""",
    ...
    sub.add_argument("-m", "--src", required=True, help="The src ID")
    sub.add_argument(
        "-s",
        "--scale_type",
        required=True,
    ...
    sub.add_argument("-i", "--id", required=True, help="The src ID")
    ...
    sub.add_argument(
        "-m",
        "--src",
        help="The src to start fine-tuning from",
    )
    sub.add_argument(
        "--suffix",
        help="If set, this argument can be used to customize the generated fine-tuned src name."
        ...
        'For example, `openai api fine_tunes.create -t test.jsonl -m ada --suffix "custom src name" '
        "could generate a src with the name "
        "ada:ft-your-org:custom-src-name-2022-02-15-04-21-04",
    )
    sub.add_argument(
        "--no_follow",
        action="store_true",
    ...
    sub.add_argument(
        "--n_epochs",
        type=int,
        help="The number of epochs to train the src for. An epoch refers to one "
    ...
    sub.add_argument(
        ...
        "depends on your use case. This determines how much the src prioritizes "
    ...
    # Required
    sub.add_argument("-m", "--src", type=str, default="whisper-1")
